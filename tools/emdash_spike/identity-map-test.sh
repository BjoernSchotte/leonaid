...
if [ -n "$(docker ps -aq --filter "label=com.docker.compose.project=$project")" ] || \
  ...
  rmdir "$proof"
  echo "emdash-identity-map: project collision; refusing" >&2
  exit 1
fi
cleanup() {
...
compose run --rm --no-deps proof node tools/emdash_spike/identity-map-proof.mjs
compose restart core-postgres
compose up --detach --wait core-postgres
compose run --rm --no-deps proof node tools/emdash_spike/identity-map-proof.mjs --existing
