...
try {
  ...
  await assert.rejects(run(), /krapfentaxi_import_existing_editorial_content/);
  ...
  const privateDirectory = await mkdtemp(join(tmpdir(), "leonaid-import-cli-"));
  try {
  ...
  const item = await repository.findById("campaign_pages", repeated.contentId);
