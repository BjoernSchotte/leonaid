...
if [ -z "${LEONAID_TEST_STACK:-}" ]; then
  exec python3 "$root/tools/testing/shared_stack.py" core tools/materials/browser.sh
fi
...
shared_services="api web pwa proxy"
...
results=${LEONAID_CI_ARTIFACT_DIR:-$root/.artifacts/material-browser}
mkdir -p "$results"
...
docker run --rm --network "${project}_edge" \
  ...
  tests/e2e/modules-materials.spec.mjs --output=/results/materials --reporter=line
