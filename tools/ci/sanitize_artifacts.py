...
# UUID suffixes can contain only digits and resemble a local phone number.
# Remove the complete canonical identifier, never arbitrary digit sequences.
UUID_PATTERN = re.compile(
    rb"(?<![\w-])[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-"
    rb"[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}(?![\w-])"
)
...
def sanitize_text(
    ...
    *,
    ...
    sanitized = UUID_PATTERN.sub(b"[UUID]", sanitized)
