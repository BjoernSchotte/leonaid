...
def main() -> int:
    ...
    for raw in content.splitlines():
        ...
        if key in existing_values:
            ...
            # Preserve the bytes of keys generated by the initial CMS spike;
            # EmDash requires its versioned base64url representation, not hex.
            if key == "CMS_ENCRYPTION_KEY" and re.fullmatch(r"[0-9a-f]{64}", existing):
                encoded = (
                    base64.urlsafe_b64encode(bytes.fromhex(existing))
                    .decode()
                    .rstrip("=")
                )
                existing = f"emdash_enc_v1_{encoded}"
            rendered_lines.append(f"{key}={existing}")
            continue
