...
- [x] **090.2d** Couple manual deletion acknowledgement and production worker cleanup to the independent archive. Prove archive-outage HTTP 503, durable exact-operation retries, unchanged original ledger/outbox identity, no cleanup during outage and real old-backup restoration after source-project removal without an explicit post-deletion export. Acceptance: **090.A3, acknowledgement portion**. Integration: **090.S3b**. Full 090.2b remains open for retention-originated interruption coverage, host-loss cutoff provenance and the remaining operator recovery contract. [Live evidence](proofs/SURV-090.md#automatic-archive-acknowledgement-and-worker-gate).
