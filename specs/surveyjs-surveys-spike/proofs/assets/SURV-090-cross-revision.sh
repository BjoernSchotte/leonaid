...
root=${1:-$(pwd)}
root=$(cd "$root" && pwd)
source_root=${2:?preceding checkout required}
...
proof=$(mktemp -d "$source_root/.artifacts/survey-restic.XXXXXX")
source_owned=false
target_owned=false
source_compose() {
  docker compose --project-name "$source_project" --env-file "$source_root/.env.local" \
    --file "$source_root/infra/compose/compose.yml" --file "$proof/source.yml" \
    --profile dev-mail "$@"
}
target_compose() {
...
cleanup() {
  ...
  for project in "$source_project" "$target_project"; do
    owned=$source_owned
    if [ "$project" = "$target_project" ]; then owned=$target_owned; fi
    if [ "$owned" != true ]; then continue; fi
    for service in api worker web pwa public survey-validator proxy; do
      tag="$project-$service:latest"
      existing=$(docker image ls --quiet --filter "reference=$tag") || status=1
      if [ -n "$existing" ]; then docker image rm "$tag" >/dev/null 2>&1 || status=1; fi
      remaining=$(docker image ls --quiet --filter "reference=$tag") || status=1
      if [ -n "$remaining" ]; then status=1; fi
    done
  done
  rm -rf "$proof"
...
for project in "$source_project" "$target_project"; do
  for service in api worker web pwa public survey-validator proxy; do
    existing=$(docker image ls --quiet --filter "reference=$project-$service:latest")
    [ -z "$existing" ]
  done
done
if [ "$mode" != manual ]; then
...
source_compose run --rm --no-deps --volume "$source_root:/repo:ro" --volume "$proof:/proof" \
...
probe() {
  ...
  shift
  probe_root=$root
  if [ "$runner" = source_compose ]; then probe_root=$source_root; fi
  probe_service=api
  if [ "$1" = durable-delete ]; then probe_service=worker; fi
  "$runner" run --rm --no-deps --volume "$probe_root:/repo:ro" --volume "$proof:/proof" \
...
# The source and target deliberately use different release images.
python3 - "$proof" <<'PY'
import pathlib, secrets, sys
password = pathlib.Path(sys.argv[1]) / 'restic-password'
...
LEONAID_COMPOSE_PROJECT="$source_project" \
  ...
  sh "$source_root/tools/backup/backup.sh" "$source_root"
...
# Build current target images before restore; never reuse source image IDs.
printf '%s\n' '{"services":{}}' > "$proof/images.json"
target_owned=true
target_compose build api worker web pwa public survey-validator proxy
...
for service in ('api', 'worker', 'web', 'pwa', 'public', 'survey-validator', 'proxy'):
    image = subprocess.check_output(['docker', 'image', 'inspect', '--format', '{{.Id}}', f'{project}-{service}'], text=True).strip()
...
result.update({
    ...
    'allBuiltServiceImageIdsMatchCurrentTargetBuild': True,
    'backupSourceRevision': '875221c1054314e83a1737e0384ee2dbad3fd398',
    'restoreTargetRevision': 'cf20bec5faeb49c11a7f6f78fde54c9bd28dc061',
...
PY
cp "$proof/restic-recovery-proof.json" "$root/.artifacts/survey-ci-review/recovery-compatibility/cross-revision-result.json"
if [ "$mode" != manual ]; then
