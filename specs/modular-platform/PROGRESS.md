...
## Rollback — Frühere API auf erweitertem Schema geprüft

`rtk proxy sh tools/schema/test.sh . code-rollback` ist mit Exit 0 bestanden (Session 60135, isoliertes Projekt leonaid-poc021-test-2137972478-41850). Der Runner migriert eine leere PostgreSQL-Datenbank bis 0041 und lädt danach unveränderte Quellstände vor Registrierung (4d05276), Tasks (07d1008) und Inbox (b46f462) aus Git-Archiven. Jeder Stand startet seinen echten FastAPI-Lifespan, liest eine persistierte Sitzung und verweigert nach tatsächlicher Kontosperrung dieselbe Sitzung. Neue Task-Daten einschließlich Fälligkeit und Zurückstellung sowie die Schema-Revision bleiben unverändert. Der Stand mit bestehender Task-Route liest den neuen Task auch über seine HTTP-Schnittstelle. Alle eigenen Testressourcen wurden entfernt.

Ruff, Formatprüfung, Shell-Syntax und Mypy mit MYPYPATH=src bestanden. Der Test verwendet die heutige gepinnte Python-Laufzeit mit früherem Quellcode; er behauptet keine Rücknahme alter Container-Images. Neue Jobs drainieren, kompatible Handler und vollständige Rollout-Abnahme bleiben offen.

## Rollback — Neuer Inbox-Auftrag bei gestoppter API verarbeitet

Im eigenen Produktionsstack leonaid-shared-32c62f415463ad67 den Worker gestoppt und über die bestehende öffentliche Python-Fachoperation einen synthetischen Fall samt Kontaktauftrag gespeichert. Danach API tatsächlich gestoppt: POST am öffentlichen Inbox-Pfad wird vom TLS-validierten Proxy mit 502/503 zurückgewiesen. Den unveränderten kompatiblen Produktionsworker gestartet, während API gestoppt bleibt. Fall 132b2ca3-0023-48cc-89fb-e96a530a2e84 und Auftrag ed345672-16ea-4691-a63c-f1723eb12196 erreichen linked/completed mit attempts 1 und manual_retry_count 0. Der normale Worker verarbeitet den gespeicherten neuen Job ohne erneute Einreichung und ohne manuelle Statusänderung.

Session 75299 endet mit Exit 0. API und Worker anschließend wieder gestartet und gesund; der wieder erreichbare Identitätsendpunkt verweigert den anonymen Zugriff korrekt mit 401. Keine Browser-Cookies geändert. Keine Daten oder Volumes gelöscht. ROLLBACK.md beschreibt den Wartungsablauf, offene zukünftige/aktive/ruhende Jobs, weiterhin aktive Produzenten und den Erhalt kompatibler Handler bzw. Forward-Fix. Der Nachweis erlaubt keinen alten Worker ohne Inbox-Handler; auch ein leerer fälliger Batch wird ausdrücklich nicht als leere Queue gewertet. Er ergänzt den separaten älteren API-/Schema-Nachweis. Kein UI-Schnitt und kein Browser-Screenshot für diesen Prozessnachweis.

## M1 — Vollständiger Operations-Gate nach Sweep-Anzeige bestanden

`rtk proxy sh tools/operations/test.sh` endet mit Exit 0 (Session 76020, Projekt leonaid-poc114-test-2137972478-41396). Die aktuelle Operations-/Worker-Implementierung einschließlich lastSuccessfulSweepAt war im gebauten Image enthalten. Reale Ausfälle von Twenty, RustFS, Mail und Worker, Queue-Grenzen, strukturierte Logs/Metriken, Mail-Dead-Letter und tatsächlicher Browser-Retry bestanden. Der Browserfall „System-Admin erkennt Ausfälle und wiederholt echten Mail-Job“ endet erfolgreich. Der Runner prüft außerdem vertrauliche Inhalte in Logs und die vorhandenen Desktop-/Mobil-Screenshots. Anschließend sind sämtliche eigenen Container, Volumes und Netzwerke entfernt. Die separate Survey-Exportgruppe und die Gesamt-CI bleiben offen.

## M3 — Verlorene Bestätigung mit anschließendem echtem Rate Limit

`tests/e2e/public-inbox-retry.spec.mjs` besteht gegen den bestehenden Produktionsstack mit echter Public-Seite/API (Session 14915, ein Test, 8,3 Sekunden). Der Browser füllt das Formular aus; der Server antwortet nach tatsächlichem Commit mit 201 und einer Referenz. Nur diese Rückantwort wird auf dem Transportweg verworfen. Weitere ungültige Requests erreichen den echten Server und lösen dessen Rate Limit aus. Die nächste Formularwiederholung erhält tatsächlich 429. Der Test vergleicht den gesamten ursprünglichen und wiederholten JSON-Auftrag einschließlich idempotencyKey, prüft weiterhin gesperrte unveränderte Eingaben, verständlichen unklaren Status und aktivierten Wiederholungsbutton.

Keine Serverantwort wird erfunden oder ersetzt. Die exakte Transportfehler-Ausnahme ist mit Quellhash und Begründung in reviewed_network_faults.json dokumentiert; no-test-doubles besteht. Es handelt sich um einen gespeicherten synthetischen Fall. Die globale Inbox-/Campaign-/Browser-CI-Abnahme bleibt offen. Der Screenshot zeigt den tatsächlichen Zustand nach HTTP 429.

## M2 — Dauerhafter Material-Browser-Gate

Neuer vorhandene Werkzeuge nutzender Runner `sh tools/materials/browser.sh`: eigener Shared-Stack, private S3-Versionierung, zwei frisch angelegte aktive Konten ohne globale Rollen, Produktions-Web/PWA und Chromium. `modules-materials.spec.mjs` prüft in beiden Shells echte Uploads von Version 1 und 2, Auswahl und tatsächlichen Browser-Download der ersten Version mit bytegenauem Vergleich, verweigerten fremden Direktzugriff ohne Titel-/Download-Leak sowie fehlenden horizontalen Überlauf bei 1440 bzw. 390 Pixeln. Im isolierten Projekt leonaid-shared-48cd452b27960a98 bestehen beide Tests in 7,4 Sekunden; Leaf endet mit Exit 0 (Session 78396). Zuvor bestanden beide Tests auch im vorhandenen Produktionsstack.

CI erhält einen materials-E2E-Shard; er verwendet den vorhandenen Shared-Stack und dieselben gepinnten Images. Screenshots und Fehlertraces liegen im CI-Artefaktverzeichnis; die private Sitzungsdatei bleibt im temporären Verzeichnis. Ruff, Mypy des Seeds, Prettier, Shellsyntax und no-test-doubles bestanden. Kein neuer Produktionsdienst oder Testframework. Weitere Modul-/Inbox-Browserfälle und Remote-CI bleiben offen.

## M1 — Queue-Vertrag zusammengeführt abgenommen

Den übergeordneten Queue-Punkt nach erneutem Code-/Nachweisabgleich abgenommen. Der aktuelle Pending-Vertrag verlangt timezone-aware Termine; PostgreSQL beansprucht fällige Events und schützt Abschluss/Fehler mit Claim-Token. OutboxWorker begrenzt explizit registrierte Handler mit asyncio.timeout, schreibt nur sichere Fehlercodes und misst Dauer monoton. Bootstrap hält Survey-Exports unter 240 Sekunden bzw. 80 Prozent der Lease. Die aktuellen realen Prüffunktionen verify_delayed_fencing und verify_handler_timeout verwenden den bestehenden ActionProgress-Handler: transaktionaler Rollback, tatsächliche Fälligkeitsgrenze, Lease-Übernahme, veralteter Claim und genau eine Projektion; tatsächliche PostgreSQL-Tabellensperre, Abbruch vor Lease-Ende, keine halbe Projektion und erfolgreicher Retry.

Abnahmebelege sind die bereits bestandenen vollständigen Outbox- und Handler-Timeout-Läufe, die dokumentierte vollständige Exportjobmessung mit 5.000 Antworten über API/Queue/Worker/Storage und der jetzt abgeschlossene aktuelle Operations-Gate (Session 76020) einschließlich realer Ausfälle, sicherem Retry und Loghygiene. Die zugehörigen Abschnitte oben enthalten Befehle und Messwerte. Historische Hinweise innerhalb früherer Teilschritte beschreiben deren damaligen Stand. Keine neue Produktionsänderung oder erneute Behauptung eines vollständigen Survey-/CI-Abschlusses in diesem Abnahmeschnitt.

## M1 — Öffentliche Survey-Fachoperationen abgenommen

Die aktuellen HTTP-Routen verwenden benannte Methoden aus SurveyService bzw. SurveyExportService. Eingaben werden bei Direktaufrufen erneut validiert; Autorisierung verbleibt in denselben konkreten Survey-/Export-Repositories. Kein zweiter Berechtigungspfad wurde eingeführt. Der echte Modulvertrag vergleicht direkte Listen mit HTTP, prüft fremde Umfragen/Aktionsgrenzen und verweigerte Veröffentlichung samt unveränderter survey_version-Tabelle. Die vollständige HTTP-/Browser-Rechtematrix verwendet denselben darunterliegenden Pfad.

Autoritative Runnerdaten im Bericht d87d297714f74746998b0c3dae32a8c1 bestätigen contracts, lifecycle und permissions jeweils mit Exit 0 auf 1e8db1a. Git-Abgleich dieses Stands mit dem heutigen HEAD: Survey-Implementierung, Bootstrap und die betrachteten Modul-/Berechtigungsverträge sind unverändert; einzige Differenz in api.py ist der zusätzlich bereits separat geprüfte PWA-Navigationsbeitrag. Die getrennten neueren Analyse- und Export-Rechteprüfungen sind ebenfalls bestanden. `rtk proxy uv run --frozen pytest -q tests/unit/test_survey_module_api.py` erneut erfolgreich: zwei Tests, einschließlich nachträglich mutierter Eingaben für Autorentätigkeit, Analyse, Antwortauswahl, Teilnahme und Export. Diese Unit-Tests ergänzen die realen Rechteprüfungen und ersetzen sie nicht.

Der übergeordnete Fachoperationspunkt ist damit abgenommen. Survey-Recovery, vollständige Exportgruppe und Remote-Gesamt-CI bleiben eigenständige offene Anforderungen.

## M2/M3 — Gemeinsamer Browser-Gate mit echten Inbox-Dateidownloads

Den Material-Runner nach tools/testing/modular_browser.sh verschoben und den CI-Shard in modules umbenannt. Derselbe vorhandene Shared-Stack prüft jetzt auch die Public-Seite; keine zusätzliche Testplattform oder dauerhafte Infrastruktur. Der private Seed erzeugt zwei zugewiesene Inbox-Fälle für das erste der beiden Konten ohne globale Rollen. Die beiden Material-Browserfälle verknüpfen die zuerst hochgeladene Dateiversion über den tatsächlichen Picker im jeweiligen Web-/PWA-Fall. Nach dem zweiten Upload laden sie aus dem Fall die ursprünglichen Bytes herunter, entfernen den Verweis und prüfen den erhaltenen Material-Versionskopf. Der dritte Fall prüft den echten öffentlichen Commit mit verlorener Rückantwort und nachfolgendem echten Rate Limit.

`rtk proxy sh tools/testing/modular_browser.sh` besteht im frischen Projekt leonaid-shared-9ec5f040624f8c81: drei Browserfälle in 15,4 Sekunden, Leaf Exit 0 (Session 28756). Screenshots für beide Inbox-Downloads und den Formularfehler liegen in .artifacts/modular-browser/modules. Ruff, Mypy des Seeds, Prettier, Shellsyntax, Diffprüfung und no-test-doubles bestanden. Der vorherige Material-Runner-Pfad in früheren Nachweisabschnitten bezeichnet dessen damaligen Stand; der aktuelle wiederholbare Einstieg ist der gemeinsame Runner. Weitere Gesamtgates bleiben offen.

## M2 — Wissensbearbeitung und Konflikterhaltung im dauerhaften Browser-Test

modules-knowledge.spec.mjs erzeugt über die reale UI eine private Seite, speichert Inhalt und öffnet sie in der jeweils anderen Shell. Eine echte zweite Sitzung speichert während der erste Editor einen lokalen Entwurf hält. Dessen veralteter Speicherversuch zeigt den Konflikt, ohne den Entwurf zu verändern. Abgelehntes Neuladen erhält ihn, bestätigtes Neuladen übernimmt den tatsächlich gespeicherten Stand. Beide Richtungen laufen mit 1440 bzw. 390 Pixeln; Prüfung auf horizontalen Überlauf eingeschlossen.

Zwei Tests gegen den bestehenden Produktionsstack bestanden (Session 21791, 11,6 Sekunden, Exit 0). Keine Serverantwort oder I/O-Implementierung ersetzt. Screenshots unter /tmp/leonaid-knowledge-browser-proof visuell geprüft. Der vorhandene gemeinsame modulare CI-Runner führt die Tests zusätzlich zu Material/Inbox/Public aus. Prettier, no-test-doubles und Diffprüfung bestanden; die Remote-Gesamtabnahme bleibt offen.

## M1 — Vollständige automatische Survey-Exportgruppe bestanden

Session 10846 endet mit Exit 0. Der zentrale Bericht a241762639374f03ac8a5db5c68c23db bestätigt alle fünf Checks der vollständigen Exportgruppe: exports (545,296 s), export-permissions (339,298 s), export-states (462,458 s), export-recovery (1143,868 s) und export-limits. Er meldet einen vollständig bestandenen automatischen Durchlauf. Die vorher in derselben Sequenz ausgeführten Analyse-, Request-Limit- und Payload-Limitchecks sind ebenfalls bereits dokumentiert bestanden.

Die Export-Recovery nutzt den echten Worker mit absichtlichem Prozessabbruch, Wiederanlauf, zwischenzeitlicher Stornierung, Rendererfehler und tatsächlichem Storage-Ausfall. Der Test erzeugt und prüft reale Artefakte und persistierte Zustände. Manuelle Render-Abnahme gemäß den im Bericht genannten Survey-Dokumenten und Remote-Gesamt-CI bleiben ausdrücklich offen; der automatische Bericht ersetzt diese nicht.

## M2 — Aufgabenablauf als dauerhafter Web-/PWA-Browsertest

`tests/e2e/modules-tasks.spec.mjs` legt eine private Liste und eine persönlich zugewiesene Aufgabe über die tatsächliche Oberfläche an. Unterschiedliche zukünftige Fälligkeits-/Zurückstellungszeitpunkte werden gespeichert. Die persönliche Ansicht blendet die zurückgestellte Aufgabe aus; die ausdrückliche Filteroption zeigt sie wieder. Nach Entfernen der Zurückstellung und Erledigen bleibt die Fälligkeit beim erneuten Öffnen unverändert. Beide Shells werden bei 1440 bzw. 390 Pixeln ohne horizontalen Dokumentüberlauf geprüft.

Der echte Produktionsstack besteht beide Fälle in 20,6 Sekunden (Session 2484, Exit 0). Frühere Versuche scheiterten an exakten Label-Selektoren, die bei verschachtelten Selects deren Optionsinhalt einschließen; die zugänglichen Combobox-Namen lösen die Auswahl eindeutig auf. Es wurden keine Produktrechte oder Assertions abgeschwächt. Screenshots unter /tmp/leonaid-tasks-browser-proof visuell geprüft; sie zeigen die erneut geöffnete erledigte Aufgabe. Prettier, no-test-doubles und Shellsyntax bestanden. Der vorhandene modulare CI-Runner enthält beide neuen Fälle. Remote-CI ist noch offen und hat unabhängig davon Seed-Typfehler sowie Artefakt-Dateirechte gemeldet.

## CI — Inbox-Browser-Seeds typisiert

Remote-Job 103791584763 auf 810989a meldet 13 Mypy-Fehler in zwei Inbox-Seeds. Der Public-Seed erhält die fehlende None-Rückgabeannotation; der interne Seed validiert die Prozessumgebung mit Settings.model_validate wie der bestehende Material-Seed. Keine Konfigurationswerte werden fest eingebaut oder ausgegeben. `PYTHONPATH=src uv run --frozen mypy src tools/inbox/browser_seed.py tools/inbox/public_browser_seed.py` besteht für 175 Quelldateien; Ruff-Check und Formatprüfung beider Seeds bestehen. Der erste lokale schmale Mypy-Aufruf hatte src nicht als Prüfziel enthalten und ist kein Produktfehler. Die separat laufende Dateirechtekorrektur des modularen Browser-Runners ist nicht Teil dieses Schnitts.

## M1 — Export-Render-Abnahme auf aktuellem Modulstand

Die Produktionsrenderer mit aktuellem Quellcode im vorhandenen API-Image erneut ausgeführt (tools/surveys/pdf_render.py und xlsx_render.py, Session 76271, Exit 0). Normal-, Langtext- und Leer-PDF sowie alle vier XLSX-Dateien behalten die dokumentierten SHA-256-Werte. Alle 16 Fixture-PDF-Seiten und vier Seiten des tatsächlichen Worker-Exports aus dem abgeschlossenen Exportlauf mit Poppler auf 1000 Pixel gerendert und vollständig visuell geprüft: Tabellenfortsetzungen mit Kontext, lange Beschriftungen, fehlende Werte und Sonderzeichen bleiben lesbar, ohne Überlagerung oder abgeschnittene Inhalte.

Die vier XLSX-Dateien sind byteidentisch zur vollständigen 46-seitigen Consumer-Abnahme in SURV-080.md. Auch die Hashes der dort aufbewahrten Consumer-PDFs stimmen mit dem alten Review-Ledger überein; diese Abnahme wird deshalb weiterverwendet, ohne einen neuen LibreOffice-Lauf vorzutäuschen. Der neue Ledger proofs/export-render-review.json hält aktuelle Quelldrevision, Artefakthashes, alle geprüften PDF-Seiten und diese Unterscheidung fest. Dies ist eine begrenzte visuelle Regression für den synthetischen Korpus, keine PDF-Barrierefreiheitszertifizierung. Die vollständige Remote-CI bleibt offen.

## CI — Modul-Browserartefakte und Dateieigentum korrigiert

Remote-Job 103791603956 hatte alle fünf damaligen Browserfälle bestanden (21,5 s), scheiterte anschließend an der ausdrücklichen Binary-Sperre des öffentlichen Artefakt-Sanitizers und beim Löschen root-eigener Dateien. Der gemeinsame Runner verwendet jetzt für Seed und Chromium die Host-UID/GID. Browserbilder und Traces liegen unter .artifacts/modular-browser bzw. LEONAID_MODULE_BROWSER_ARTIFACT_DIR, getrennt vom öffentlichen CI-Logpaket; die Sanitizer-Regeln bleiben unverändert. Geprüfte Screenshots werden separat per PR-Kommentar bereitgestellt.

Der vollständige aktuelle Lauf mit sieben echten Browserfällen besteht in 35,4 Sekunden (Session 28734, Exit 0, Projekt leonaid-shared-b8d505105a8773c2). Wissen-Konflikte, Materialversionen mit Inbox-Download, Aufgaben-Zeitpunkte und öffentlicher Retry nach echtem Rate Limit funktionieren in den jeweils vorgesehenen Shells. Seed-Dateirechte bleiben 0600; sämtliche eigenen Container, Volumes und Netze wurden entfernt. Shellsyntax und Diffprüfung bestanden. Die übrigen Remote-Gates bleiben offen.

## M2 — Akquise-Weiterleitung bei registrierten Arbeitsmodulen erhalten

Der bestehende Web-Guard nahm nur Surveys aus seiner Verwaltungsnavigation aus. Durch die neuen allgemein sichtbaren Modulbeiträge blieb Anna bei einem direkten Aufruf von /admin/invoices im Web-Bereich. Der Guard nimmt nun alle explizit registrierten Modul-IDs aus dieser bestehenden Verwaltungsentscheidung aus. Direkte Modulrouten bleiben erreichbar; ihre Fachrechte prüft weiterhin der Server. Es gibt keine zweite Liste von Modulnamen und keine Lockerung der Dokumentberechtigungen.

`python3 tools/testing/shared_stack.py golden tools/documents/test.sh` besteht vollständig (Session 31347, Exit 0; Projekt leonaid-shared-660b68f7856b26ce). Beide Browserfälle bestehen in 6,3 Sekunden: Admin-Download byteidentisch, Finanzansicht mobil, Anna wird nach /app/ mit persönlicher Begrüßung weitergeleitet, Akquise und Fremdaktion erhalten keine Rechnungsbytes. Der abschließende echte RustFS-Fehlobjektvertrag und vollständige Bereinigung bestehen ebenfalls. Screenshot .artifacts/poc093/document-acquirer-denied.png visuell geprüft. Web-TypeScript-Prüfung und Prettier bestanden. Der erste lokale Aufruf mit Core-Fixture scheiterte vor der Browserprüfung an fehlenden Golden-PDFs; er wurde nicht als Produktnachweis gewertet. Remote-Gesamt-CI bleibt offen.

## M2 — Gemeinsame Web-/PWA-Oberflächen zusammengeführt abgenommen

Den übergeordneten Navigations-/Mobile-Punkt nach Abgleich des aktuellen Codes und der tatsächlichen Browsernachweise geschlossen. Die Shell-Listen registeredModules und registeredPwaModules referenzieren dieselben Renderbeiträge aus packages/features; Tasks, Wissen und Materialien laden ihre jeweils eine gemeinsame Seite lazy. Die unterschiedlichen Routen bleiben Parameter derselben Implementierung. Die vorhandenen serverseitigen Objekt- und Rollenprüfungen bleiben maßgeblich.

Die dokumentierten Rechte-/Direktlink- und Suchprüfungen werden durch den aktuellen siebenfachen Produktions-Browserlauf ergänzt: beide Shells, 390/1440 Pixel, Wissenskonflikte mit Entwurfserhalt, Materialupload und bytegenauer Download alter Version aus der Inbox sowie persönliche Task-Zuständigkeit, getrennte Zeitpunkte und Erledigen. Der vollständige Dokumentengate bestätigt zusätzlich die erhaltene Akquise-Weiterleitung und fremden Zugriffsschutz nach der Guard-Korrektur. Die früheren manuellen Nachweise für Referenzpicker, Freigabe/Entzug und mobile Editor-Erreichbarkeit stehen weiterhin in den jeweiligen Abschnitten. Damit ist M2 funktional abgenommen; die übergreifende vollständige Remote-CI aus Abschnitt 8 bleibt offen.

## M3 — Öffentliche Routenregression und Oberflächenabnahme abgeschlossen

Der alte Public-Vertrag setzte fehlende Bestellfreigabe mit fehlendem Formular gleich. Die neue veröffentlichte Aktionsseite darf unabhängig davon Anfragen an die Inbox entgegennehmen. HTTP-Vertrag und bestehender Browserfall prüfen nun ausdrücklich dieses Anfrageformular und sein einziges Script, während das Bestellformular bei gesperrten Bestellungen fehlt. Archive und inaktive Seiten bleiben ohne Formular; die vorhandene Script-Freiheit dieser Seiten wird weiterhin geprüft. Keine Produktionsvalidierung oder Browserassertion wurde entfernt, um einen Fehler zu verbergen.

`python3 tools/testing/shared_stack.py golden tools/public_actions/test.sh` besteht vollständig (Session 59688, Exit 0; Projekt leonaid-shared-4d3333ca122076f0). Der echte HTTP-Vertrag prüft Folgejahrwechsel, neutrale Aliase, Archive, Angebotsdaten und Schreibschutz. Aktive/inaktive/archivierte Seiten bestehen in Chromium, Firefox und WebKit; die zusätzliche 320–1440px-Matrix läuft nur in Chromium (vier bestandene, zwei dafür planmäßig übersprungene Fälle). Der separate Public-Audit besteht (LCP 340 ms, INP 24 ms, CLS 0,0003 im dokumentierten Profil); alle eigenen Ressourcen sind entfernt. Screenshots unter .artifacts/poc071 visuell geprüft. Ruff, Mypy, Prettier, no-test-doubles und Diffprüfung bestanden.

Damit ist der übergeordnete M3-Oberflächenpunkt zusammen mit den zuvor dokumentierten Web-/PWA-Fallabläufen, Rechtematrizen, echten Twenty-Ausfall-/Wiederanläufen, CMS-Publish, kanonischer Campaign, Alias-Weiterleitung, verlorener Commit-Bestätigung und den dauerhaften Inbox-Dateidownloads abgenommen. Diese Nachweise bleiben getrennt nachvollziehbar; der Public-Katalogtest allein beweist nicht die gesamte Inbox. Die übergreifende Remote-CI aus Abschnitt 8 bleibt offen.

## CI — UUID-Ende nicht als Telefonnummer behandeln

Der Remote-Schema-/Outbox-Job 103796504622 auf c6dcaae bestand einschließlich Crash-Recovery, Fencing, Retry und vollständiger Ressourcenbereinigung. Erst der Artefakt-Sanitizer verweigerte ein numerisches UUID-Ende in Materialrouten als Telefon-Signatur. Vollständige kanonische UUIDs werden nun vor der unveränderten PII-Prüfung redigiert. Der Regressionstest belegt die betroffene Route und blockiert weiterhin eine daneben stehende Telefonnummer. Die vollständige Sanitizer-Suite und die tatsächliche Remote-Testausgabe bestehen lokal; Ruff und Diffprüfung ebenfalls. Der separate Identity-Gate und die zusätzlich laufende Nightly-Abnahme bleiben offen.

## CI — Identitätsvertrag mit PWA-Survey-Beitrag abgeglichen

Die Survey-Registrierung bietet denselben bestehenden Web-Editor auch aus der PWA an. Der strikte Navigationsvertrag erwartete diesen Beitrag bisher ausschließlich im Web. WORK_MODULES enthält Surveys jetzt für beide Oberflächen; die exakten Mengenprüfungen und alle Rollen-/Objektschutzprüfungen bleiben bestehen. Keine Produktionsrechte wurden geändert.

Der vollständige isolierte Lauf `python3 tools/testing/shared_stack.py core tools/identity/test.sh` besteht (Session 47042, Exit 0). HTTP-Verträge für Rollen, Statusrevision, Replay, Konkurrenz und Sitzungsentzug sowie alle zehn Chromium-Fälle bestehen (22,7 Sekunden). Anna erreicht ihre Akquise-Ansicht und wird aus nicht erlaubten Verwaltungsrouten korrekt weitergeleitet. Screenshot acquirer-admin-redirect.png zeigt zusätzlich die erhaltenen Einträge Meine Sponsoren, Bestellung erfassen und Neues neben den Modulbeiträgen. Beide Anna-Screenshots wurden visuell geprüft. Das eigene Projekt leonaid-shared-8f05aa5d161de689 ist vollständig bereinigt. Ruff und die zuvor ausgeführte Mypy-Prüfung über 174 Quelldateien bestehen; Remote-Gesamt-CI bleibt offen.

## Betrieb — Privates Backup-Manifest für Restic lesbar erzeugen

Nightly 34784494755 auf c6dcaae zeigt in fünf Backup-/Upgrade-Jobs ein als root erzeugtes Manifest mit Dateimodus 0600. Restic beziehungsweise die Manifest-Ausgabe laufen unter der Host-UID und können diese Datei unter Linux nicht lesen. Der Manifest-Prozess verwendet nun dieselbe Host-UID/GID. Die privaten Dateirechte und die Manifest-Inventarprüfung bleiben unverändert.

Der echte lokale Lauf (Session 9458, Projekt leonaid-poc112-source-2137972478-63047) sichert alle fünf Dateien einschließlich Manifest, prüft beide Restic-Packs vollständig ohne Fehler und startet die zuvor laufenden Quelldienste erfolgreich wieder. Die separate Restore-Phase scheitert anschließend am vorab durch den Testrunner reservierten Zielnetzwerk; der Gesamtlauf endet deshalb mit Exit 1 und zählt ausdrücklich nicht als vollständiger Restore-Nachweis. Quell-, Ziel- und Wrapper-Ressourcen sind bereinigt. Shellsyntax und Diffprüfung bestehen. Die Reihenfolgekorrektur im Restore-Test wird getrennt geprüft; Remote-Nightly-Abnahme bleibt offen.

## CI — Native Formularvalidierung im Golden-Zahlungsablauf diagnostizieren

Die Remote-Golden-Jobs 103798972423 und 103801819104 scheitern in WebKit nach dem Zahlungsklick: Der Zustand bleibt outstanding; ein Zahlungsrequest für den betroffenen Fall fehlt im Service-Log. Die Ursache ist damit noch nicht festgestellt. Der Browser-Gate prüft nun unmittelbar vor dem unveränderten Klick die native Validität aller Zahlungsfelder. Bei Fehlern werden ausschließlich Testfeld-IDs und boolesche Validitätsflags ausgegeben. Die bestehende paid-Assertion und alle fachlichen Nachbedingungen bleiben bestehen.

`LEONAID_GOLDEN_PART=primary python3 tools/testing/shared_stack.py golden tools/golden_journey/test.sh` besteht vollständig (Session 7479, Exit 0; Projekt leonaid-shared-e8ad65a296976829). Beide Runden bestehen in Chromium, Firefox und WebKit (44,7 und 43,9 Sekunden Browserzeit); Vertragsprüfung und vollständige Ressourcenbereinigung sind erfolgreich. Zwölf Browserbilder wurden vor der regulären temporären Bereinigung separat gesichert; WebKit-Desktop und mobile Mitgliederansicht wurden visuell geprüft. Prettier und Diffprüfung bestehen. Der vorherige lokale Aufruf ohne LEONAID_GOLDEN_PART scheiterte vor Browserstart an der ausdrücklichen Runner-Voraussetzung; auch dessen Ressourcen sind entfernt. Lokaler Erfolg wird nicht als Behebung des Remote-Fehlers ausgegeben; der nächste CI-Lauf erhält jetzt die genauere Diagnose.

## CI — Synthetische Golden-Fehlerbilder getrennt bereitstellen

Auf abb300d bestehen alle regulären PR-Gates außer den beiden Golden-Installationen und ihrem Sammelcheck. Die native Formularvalidität besteht auch in diesen fehlgeschlagenen WebKit-Läufen; die Ursache des fehlenden Zahlungsrequests bleibt offen. Der lokale AMD64-Vergleich besteht bislang in der ersten Runde in allen drei Browsern und ersetzt den Remote-Nachweis nicht.

Der Golden-Workflow veröffentlicht bei Fehlern nun ausschließlich `test-failed-*.png` aus seinem synthetischen Browserlauf als separates, sieben Tage aufbewahrtes Artefakt. Dies ermöglicht die vom Nutzer gewünschten geprüften Screenshot-Kommentare und die visuelle Untersuchung des CI-Fehlers. Traces, Sitzungsdateien und private Textausgaben werden nicht von diesem Upload erfasst; der bestehende Log-Sanitizer bleibt unverändert. Der vorhandene Workflow-Regressionstest deckte auf, dass eine globale Upload-Anzahl fehlende Pflicht-Uploads verdecken kann. Die Prüfung verlangt diese nun ausdrücklich je ausführendem Job, einschließlich Golden-Teiljobs statt ihres Sammelchecks. `python3 tools/ci/workflow_contract_test.py` besteht. Der tatsächliche Remote-Bildupload wird im folgenden CI-Lauf geprüft; der Golden-Fehler ist noch nicht behoben.

## CI — Vollständiger lokaler AMD64-Browservergleich

Der isolierte Golden-Primärlauf mit explizitem `linux/amd64`-Playwright-Container besteht vollständig (Session 55881, Exit 0; Projekt leonaid-shared-5d176ec7a352825d). Verwendet wurde eine temporäre Kopie des bestehenden Golden-Runners mit ausschließlich diesem zusätzlichen Docker-Plattformparameter; Backend und Worker liefen weiterhin nativ auf ARM. Beide Runden bestehen jeweils in Chromium, Firefox und WebKit (2,0 und 2,1 Minuten Browserzeit), einschließlich nachgelagerter Vertragsprüfung und vollständiger Entfernung der eigenen Container, Volumes und Netze. Zwölf Browserbilder wurden gesichert; WebKit-Verwaltung und mobile Mitgliederansicht visuell geprüft und in PR-Kommentar 5656717970 angehängt. Der CI-WebKit-Fehler ist damit auch unter lokaler AMD64-Emulation nicht reproduziert und bleibt offen.

## CI — Angeforderte Golden-PNGs an der Uploadgrenze freigeben

Der Security-Gate auf 37e5d14 weist den neuen Screenshot-Pfad zunächst korrekt als nicht freigegeben ab. Die explizite Freigabeliste enthält nun genau `.artifacts/golden-journey-browser/**/test-failed-*.png`. Der echte Grenztest bestätigt den erlaubten Pfad und verweigert weiterhin den gesamten Browserordner, beliebige PNGs, Traces und Fehlerkontextdateien. `python3 -m tools.pilot.test` besteht einschließlich Dateirechten, Git-Index/-Historie und allen Workflow-Uploads; Ruff ebenfalls. Die zusätzlichen Remote-Downloadfehler mit HTTP 429 bleiben davon getrennt zu prüfen.

Auf demselben Remote-Lauf 34787923947 bestehen anschließend beide Golden-Installationen (Jobs 103806712966 und 103806712970) sowie der Vergleich 103807434356. Es wurde keine Änderung am Zahlungsablauf vorgenommen; dieser einzelne grüne Lauf erklärt den zuvor sporadischen WebKit-Fehler nicht. Die gezielte Screenshot-Erfassung bleibt für ein erneutes Auftreten verfügbar.

## CI — Fehlgeschlagenen Golden-Zahlungsklick im Browser eingrenzen

Auf 075de20 besteht die Primärinstallation; Wiederholung 103807659319 scheitert erneut in WebKit beim erwarteten Zahlungsstatus. Der nun verfügbare Screenshot-Upload 10326499611 enthält vier Fehlerbilder. Die Rechnungsansicht zeigt gültig ausgefüllte Felder, keine sichtbare Fehlermeldung und den Submit-Button am unteren Fensterrand. Das beweist noch keine Ursache. Der unveränderte Zahlungsklick erhält deshalb eine ausschließlich im Fehlerfall ausgegebene, auf 24 Einträge begrenzte Ereignisliste mit Ereignistyp, Test-ID/Elementtyp und Prevented-Flag. Eingabewerte und Sitzungsdaten werden nicht erfasst. Die ursprüngliche paid-Assertion bleibt maßgeblich. JavaScript-Syntax und Prettier sind geprüft; tatsächliche Ereignisdiagnose folgt im nächsten CI-Lauf.

## CI — Fixture-Cache-Ausfall mit regulärem Neuaufbau auffangen

Die Runs 34787923947 und 34788736635 zeigen wiederholte HTTP-429-Antworten beim Download der synthetischen Fixture. Betroffene Contract-/Security-/Integrationsjobs scheitern vor ihrem Fachtest. Die Composite-Action verlangt weiter den exakten aktuellen Fixture-Schlüssel, baut bei fehlendem Cachetreffer aber mit dem vorhandenen `ci_fixture.py build` in einem separaten Jobverzeichnis neu. Vor Aktivierung gelten dieselben Hash-/Inventarprüfungen. Kein alternativer Cache-Schlüssel, kein Überschreiben unvollständiger Downloads und kein Überspringen eines Fachtests. Die sechs vorhandenen Fixture-Tests, der vollständige Pilot-Upload-Grenztest und Prettier bestehen. Der tatsächliche Neuaufbau nach Cache-Ausfall bleibt im folgenden CI-Lauf nachzuweisen.

## M1 — Manuellen Survey-Restic-Recovery-Pfad vollständig abgenommen

`sh tools/surveys/restic_recovery.sh . manual` besteht mit Exit 0 (Session 38571; Quellprojekt leonaid-poc112-surveys-2137972478-74685). Echtes verschlüsseltes Backup, Rotation und vollständige Pack-Prüfung bestanden. Nach tatsächlichem Entfernen der Quelle stellt der erste Zielversuch die alten SQL-Daten und exakte Exportversion wieder her, verweigert aber ohne unabhängigen Lösch-Checkpoint den Start der Writer. Der zweite Restore spielt den gültigen Checkpoint ein, entfernt den nach Backup gelöschten Inhalt und startet ausschließlich die ursprünglichen Image-IDs. Alte Sitzung und öffentliche Route können den gelöschten Inhalt und Export danach nicht mehr lesen.

Der Chromium-Infrastrukturtest besteht in 1,2 Sekunden; sein Public-Screenshot wurde vor der temporären Bereinigung gesichert und visuell geprüft. Alle eigenen Quell-/Zielcontainer, Volumes und Netzwerke sind entfernt. Das maschinenlesbare lokale Ergebnis liegt unter `.artifacts/surveys-restic/restic-recovery-proof.json`. Der manuell separat aufbewahrte Checkpoint und das lokale verschlüsselte Repository sind die ausdrücklich geprüfte Grenze; unabhängige Archiv-/Durable-Varianten, Pilot-Wrapper und erweiterte Remote-Nightly-Abnahme bleiben offen.

## Betrieb — Vollständiger Backup-/Restore-Gate mit isolierten Testnetzen

`sh tools/backup/test.sh` besteht vollständig mit Exit 0 (Session 11975; Quelle leonaid-poc112-source-2137972478-77145). Der Testrunner reserviert seine acht Restore-Netze unter einem getrennten Fixture-Eigentümer und referenziert sie im Ziel-Compose als externe Netze. Auch der Browser verbindet sich mit diesem tatsächlichen Fixture-Netz. Die produktive Restore-Sperre für nicht leere Zielprojekte bleibt unverändert; die Bereinigung prüft Quelle, Ziel und Fixture-Netze getrennt.

Sicherung, Passwort-/Klartext-/Zielschutzprüfungen, vollständige Restic-Integrität, logischer und bytegenauer Inventarvergleich einschließlich Core, Twenty und Dateiobjekten sowie Golden-Snapshot bestehen. Der abschließende Chromium-Test bestätigt die arbeitsfähige Instanz mit bestehender Sitzung (2,4 s). RPO 329 Sekunden und RTO 634 Sekunden liegen innerhalb der vorhandenen Ziele; maschinenlesbarer Bericht und geprüfter Screenshot liegen unter `.artifacts/poc112/`. Alle eigenen Ressourcen wurden entfernt. Der vorherige Lauf 12111 bestand bereits Restore und Inventarvergleich, scheiterte aber vor Browserstart an dessen alter Netzwerkreferenz (Exit 125); dessen vollständige Bereinigung wurde separat verifiziert. Der jetzige vollständige Lauf ersetzt diesen Teilnachweis. Remote-Nightly bleibt offen.

## UI — Global erzwungenes sanftes Scrollen entfernen

Die echte CI-Ereignisdiagnose auf 2bcfd51 (Job 103809942177) zeigt `pointerdown` auf `record-payment`, anschließend `pointerup` und `click` auf `payment-form`; es entsteht kein Submit. Die gültigen Felder und der ausbleibende Request sind damit durch ein verlorenes Klickziel eingegrenzt. Die App-Shell erzwingt nun kein globales `scroll-behavior: smooth` mehr. Formularbedienung verwendet das unmittelbare native Scrollverhalten; Zahlungslogik, Validierung und ursprüngliche Browserassertionen bleiben unverändert. Die Diagnose bleibt für einen erneuten Fehler verfügbar.

`LEONAID_GOLDEN_PART=primary python3 tools/testing/shared_stack.py golden tools/golden_journey/test.sh` besteht vollständig (Session 72824, Exit 0; Projekt leonaid-shared-126b049c4c60cecb). Beide Runden bestehen in Chromium, Firefox und WebKit (45,9 und 44,9 Sekunden), einschließlich Vertragsprüfungen und vollständiger Ressourcenbereinigung. Sechs Bilder der ersten Runde wurden separat gesichert; die WebKit-Verwaltungsansicht wurde visuell geprüft. Prettier und Diffprüfung bestehen. Da der Fehler zuvor lokal sporadisch nicht reproduzierbar war, bleibt die anschließende Remote-Abnahme für die Wirkung dieser Korrektur maßgeblich.

## Betrieb — Backup-Wiederaufnahme mit CI-Compose kompatibel

Nightly 34790095327 scheitert nach erfolgreicher Sicherung beim Wiederanlauf: Compose 2.38.2 des CI-Runners kennt die verwendeten Optionen von `start` nicht. Die Bereinigung verwendet jetzt `up --no-deps --no-recreate --no-build --wait --wait-timeout 420` für ausschließlich zuvor laufende Dienste. Ein isolierter Lauf mit dem offiziellen, SHA256-geprüften Compose-2.38.2-Binary und dem tatsächlichen Cleanup-Code bestätigt gesunde Dienste bei unveränderten Container-IDs; alle eigenen Testcontainer sind entfernt. Der Regressionstest prüft weiterhin Fehlerweitergabe, Erhalt ursprünglicher Fehler, temporäre Bereinigung und private Ausgaben; Shellsyntax besteht. Dies ist ein gezielter Kompatibilitätsnachweis, kein Ersatz für den folgenden vollständigen Remote-Recovery-Lauf.

CI 34790135355 auf a326ff0 bestätigt beide Golden-Installationen und ihren Vergleich sowie sämtliche E2E-Gruppen nach der Scrollkorrektur. Der einzelne fehlgeschlagene Integrations-Shard seed-reset bleibt getrennt zu untersuchen; die Gesamt-Abnahme ist weiterhin offen.
...
## M1 — Erweiterte Restic-Abnahme im CI bestanden

Nightly 34790776237 auf 3c5de25 besteht den vollständigen Shard recovery-restic (Job 103814471011). Das heruntergeladene Ergebnis eca70e27c7d54248a3e3176ae5c16b27 bestätigt drei tatsächlich ausgeführte Checks mit jeweils Exit 0: restic-manual, restic-archive und restic-durable. Der bestehende Runner prüft Quellverlust, unabhängige Lösch-Checkpoints vor Writer-Freigabe, entfernte SQL-Daten und exakte Exportversionen sowie die jeweils zusätzlichen Archiv-/Bestätigungsgrenzen. Browserprüfung und vollständige Bereinigung gehören zum erfolgreichen Runner. Die lokale Modellgrenze unabhängiger Archivspeicherung bleibt erhalten; kein zusätzlicher physischer Remote-Host wird behauptet. Pilot-Wrapper und Upgrade-Abnahme bleiben separat offen.

Auf b345cfe bestehen Haupt-CI 34791554227 und reguläre Survey-Abnahme 34791554187 vollständig, einschließlich beider Golden-Installationen, Seed-Reset und Parallelitätstest. Der echte lokale Twenty-Runner 78433 besteht ebenfalls einschließlich identischer wiederholter Provisionierung, absichtlich erzeugtem Drift, erlaubten/verweigerten Key-Rechten und Entfernung aller eigenen Ressourcen (leonaid-poc030-test-2137972478-88242). Die noch uncommittete Änderung an den Restore-Testnetzen ist von diesen Remote-Ergebnissen nicht abgedeckt.

## CI — Proxy-Diagnose bei Rechnungs-Browserfehlern

Haupt-CI 34792321541 auf eacb584 besteht im zweiten Versuch vollständig. Im ersten Versuch scheiterte ausschließlich der Rechnungs-Browserstart mit `net::ERR_SSL_PROTOCOL_ERROR`; die vorangehenden fachlichen Rechnungsverträge bestanden. Ein einmaliger erneuter Lauf der fehlgeschlagenen Jobs besteht ohne Änderung der Assertions. Die Ursache des TLS-Fehlers ist damit nicht belegt und wird nicht als behoben behauptet.

Der bestehende Fehlerzweig des Rechnungsrunners erfasst zusätzlich die begrenzten Proxy-Logs, damit ein erneutes Auftreten diagnostizierbar wird. Shellsyntax und Diffprüfung bestehen. Keine Änderung am Produkt oder an der Testabnahme; kein zusätzlicher Browsernachweis für diese reine Diagnoseänderung. Die vollständigen lokalen Upgrade- und Pilot-Recovery-Läufe sind weiterhin aktiv.

## M0 — Abhängigkeiten zusammengesetzter API-Module vollständig deklarieren

Der Abschlussabgleich mit Abschnitt 4.1 findet eine fehlende Startbedingung: Inbox und Wissen verwenden die öffentlichen Task- und Materialoperationen, aber die API-Registrierung deklarierte nur Wissen → Tasks. Beide Beiträge verlangen jetzt Tasks und Materialien. Die tatsächliche Produktionsregistrierung wird für jede der vier Kombinationen ohne den benötigten Beitrag geprüft: Der Start scheitert vor dem Anfügen von Routen; mit vollständigen Abhängigkeiten besteht die Validierung. Keine zusätzliche Registry oder dynamische Auflösung.

`python -m pytest -q tests/unit/test_architecture_boundaries.py tests/unit/test_module_registration.py` besteht mit 31 Tests (Session 49701, Exit 0). Ruff, Formatprüfung, No-Test-Doubles und Diffprüfung bestehen. Die Änderung betrifft die Startvalidierung; keine neue Oberfläche und kein neuer Browser-Screenshot. Commit und Push warten auf den bereits laufenden manifestgebundenen Pilot-Test, dessen HEAD während der Ausführung unverändert bleiben muss. Dessen Images enthalten diese nachträgliche Registrierungsergänzung noch nicht; die folgende CI muss den neuen Commit prüfen.

## Betrieb — Vollständiger Upgrade- und Rollback-Nachweis

`sh tools/upgrade/test.sh` besteht vollständig (Session 8949, Exit 0; Quellprojekt leonaid-poc113-upgrade-2137972478-90338). Die reservierten Restore-Netze gehören nun einem getrennten Fixture-Projekt. Dadurch bleibt die produktive Sperre für ein nicht leeres Restore-Ziel unverändert wirksam. Backup und Upgrade verwenden für diese vorhandene Reservierung denselben kleinen Helper; Browser verbinden sich mit dem tatsächlichen Fixture-Netz.

Der Lauf aktualisiert Twenty von 2.23.2 auf 2.24.0 und RustFS von 1.0.0-beta.10 auf beta.11. Er besteht alle drei Restore-Generationen, erkennt die absichtlich ungültige Core-Migration und stellt nach absichtlichen Änderungen aller vier Datenkomponenten den gesicherten Zustand wieder her. Golden Journeys vor Upgrade, nach Upgrade und nach Rollback bestehen jeweils in Chromium, Firefox und WebKit. Manifest-Promotion, Wartungs-Schreibgrenze, Release-Ledger und fachliche Gleichheit nach Rollback sind bestätigt. `.artifacts/poc113/result.json` enthält ausschließlich bestandene Teilnachweise.

Die neun Container-/Volume-/Netzinventare von Quelle, Restore-Ziel und Fixture-Projekt wurden nach Exit 0 unabhängig als leer bestätigt. Die echten Browserbilder vor/nach Upgrade und nach Rollback sind am Draft-PR angehängt; der vollständige Nachweis steht in [Kommentar 5657888720](https://github.com/BjoernSchotte/leonaid/pull/7#issuecomment-5657888720). Commit und Push dieser geprüften Runner-Korrektur folgen nach dem noch laufenden manifestgebundenen Pilot-Test. Der neue Remote-Nightly-Lauf bleibt eine separate offene Abnahme.

## M1 — Vollständiger Pilot-Release und Survey-Recovery bestanden

`sh tools/pilot_deployment/test.sh . surveys` besteht vollständig mit Exit 0 (Session 78399; Lauf 2137972478-97073 auf unverändertem HEAD 1ee0339). Der Runner baut seine vorhandenen sechs Images vor der Backup-Bucket-Anlage und verwendet dafür das Core-Image mit seinen Linux-Abhängigkeiten statt einer macOS-Host-Venv. Deploy, Staging-Freigabe, verschlüsseltes S3-Backup mit Integritätsprüfung, Wartung, benannte Alembic-Migration und manifestgebundener Release bestehen.

Nach vollständigem Entfernen der Quelle verweigert der tatsächliche Pilot-Restore fehlende, manipulierte, mit falschem Schlüssel signierte, installationsfremde und veraltete Checkpoints. Jeder Fall wird an frisch wiederhergestellten SQL-Antworten und der exakten Exportversion offline geprüft. Unterbrochene Löschanwendung wird ohne erneuten Datenimport fortgesetzt. Mit gültigem Checkpoint startet der Restore die ursprünglichen Images buildfrei; die alte Sitzung und öffentliche Route können gelöschte Inhalte und Exporte nicht mehr abrufen. Alle vier gesicherten Datenkomponenten sind erhalten. Nach Exit 0 sind die zwölf Container-/Volume-/Netzinventare von Quelle, Ziel, Build und Backup-Operator unabhängig als leer bestätigt.
