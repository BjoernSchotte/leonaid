...
8. Existing orders retain unknown/null delivery details and windows; never fabricate historical selections. Existing confirmed/invoiced orders remain readable and valid. Existing draft/review-ready orders require explicit completion before future delivery-sensitive transitions once scheduling is enabled. Provide an authorized completion path for these orders; do not make missing fields impossible to repair. Historical completion was explicitly approved by the user: a Charity Admin may select an existing configured window of the same action whose end has passed, including a retired window, only with an explicit confirmation that this is a historical delivery record. Derive the immutable snapshot on the server, include confirmation in idempotency and audit evidence, and never infer a selection automatically. Preserve already booked windows. This exception belongs only to completion of existing draft/review-ready orders; Anna's new-order capture and public Astro/EmDash order submission continue to reject past and retired windows. The completion UI must clearly separate historical confirmation from future selection and provide no preselected historical window.
...
- [ ] Reconcile with EmDash EMS-010/050/085 and extend the actual `apps/campaign-site` form renderer and serving-app transport once available. Reuse shared definitions/components where appropriate; verify both canonical campaign and alias entry points rather than assuming parity from copied markup.
      The internal resolver inspected at `6f231b7` separates canonical `route_value` (archive slug) from `order_alias` (token/submission binding). Preserve that distinction in the agreed HTTP contract and project delivery rules for the resolved action through Core. Prove differing slug/alias, alias reassignment and publication withdrawal as detailed in ACCEPTANCE.md; the legacy response serializer rejects campaign routes and cannot be reused unchanged.
...
| Selection safety | Missing, retired, past, and foreign-action windows fail for new complete delivery orders; explicit admin historical completion accepts only ended same-action configured windows; concurrent retirement cannot produce a new invalid booking |
