...
| Template/default compatibility and all non-delivery orders | Persisted Krapfentaxi/blank defaults and existing null records are proven. `form_configuration.py` now also creates and reads a real review-ready sponsoring order on a blank action, preserving EUR 5 pricing and billing with null delivery fields; exact replay returns the same order. Broader channel/template compatibility still needs integrated acceptance. | Partial |
