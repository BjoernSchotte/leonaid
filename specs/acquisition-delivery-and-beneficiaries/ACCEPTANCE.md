...
| Recovery behavior | Public stale policy/window, empty availability, no-JavaScript error retention and accepted-response retry; Anna accepted-response 503 retry; admin revision reconciliation all have recorded live proofs. Buyer-switch and action-switch reset now have actual browser proofs. Audit remaining draft and no-JavaScript unknown-outcome scope before marking the broad item complete. | Partial |
